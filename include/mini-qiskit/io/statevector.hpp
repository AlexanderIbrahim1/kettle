...
inline auto endian_to_string(mqis::QuantumStateEndian endian) -> std::string
{
...
inline auto string_to_endian(const std::string& endian) -> mqis::QuantumStateEndian
{
    using QSE = mqis::QuantumStateEndian;
...
{
    outstream << "ENDIANNESS: " << impl_mqis::endian_to_string(endian) << '\n';
    outstream << "NUMBER OF STATES: " << state.n_states() << '\n';

    for (std::size_t i {0}; i < state.n_states(); ++i) {
        outstream << impl_mqis::format_complex(state[i]) << '\n';
    }
}
...
inline auto load_statevector(std::istream& instream) -> QuantumState
{
    ...
    const auto endian = [&]() {
        ...
        return impl_mqis::string_to_endian(endian_str);
    }();
