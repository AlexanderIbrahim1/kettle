...
inline auto format_matrix2x2_(const mqis::Matrix2X2& mat) -> std::string
{
...
inline auto format_one_target_gate_(const mqis::GateInfo& info) -> std::string
{
...
inline auto format_one_control_one_target_gate_(const mqis::GateInfo& info) -> std::string
{
    const auto gate_name = PRIMITIVE_GATES_TO_STRING.at(info.gate);
...
inline auto format_one_target_one_angle_gate_(const mqis::GateInfo& info) -> std::string
{
    const auto gate_name = PRIMITIVE_GATES_TO_STRING.at(info.gate);
...
inline auto format_one_control_one_target_one_angle_gate_(const mqis::GateInfo& info) -> std::string
{
    const auto gate_name = PRIMITIVE_GATES_TO_STRING.at(info.gate);
...
inline auto format_m_gate_(const mqis::GateInfo& info) -> std::string
{
    const auto gate_name = PRIMITIVE_GATES_TO_STRING.at(info.gate);
...
inline auto format_u_gate_(const mqis::GateInfo& info, const mqis::Matrix2X2& mat) -> std::string
{
    const auto gate_name = PRIMITIVE_GATES_TO_STRING.at(info.gate);
    const auto target = unpack_single_qubit_gate_index(info);
...
inline auto format_cu_gate_(const mqis::GateInfo& info, const mqis::Matrix2X2& mat) -> std::string
{
    const auto gate_name = PRIMITIVE_GATES_TO_STRING.at(info.gate);
    const auto [control, target] = unpack_double_qubit_gate_indices(info);
...
inline void write_tangelo_circuit(const mqis::QuantumCircuit& circuit, std::ostream& stream)
{
    ...
    for (const auto& ginfo : circuit) {
        if (gid::is_one_target_transform_gate(ginfo.gate)) {
            stream << impl_mqis::format_one_target_gate_(ginfo);
        }
        else if (gid::is_one_control_one_target_transform_gate(ginfo.gate)) {
            stream << impl_mqis::format_one_control_one_target_gate_(ginfo);
        }
        else if (gid::is_one_target_one_angle_transform_gate(ginfo.gate)) {
            stream << impl_mqis::format_one_target_one_angle_gate_(ginfo);
        }
        else if (gid::is_one_control_one_target_one_angle_transform_gate(ginfo.gate)) {
            stream << impl_mqis::format_one_control_one_target_one_angle_gate_(ginfo);
        }
        else if (ginfo.gate == G::M) {
            stream << impl_mqis::format_m_gate_(ginfo);
        }
        else if (ginfo.gate == G::U) {
            const auto matrix_index = impl_mqis::unpack_gate_matrix_index(ginfo);
            stream << impl_mqis::format_u_gate_(ginfo, circuit.unitary_gate(matrix_index));
        }
        else if (ginfo.gate == G::CU) {
            const auto matrix_index = impl_mqis::unpack_gate_matrix_index(ginfo);
            stream << impl_mqis::format_cu_gate_(ginfo, circuit.unitary_gate(matrix_index));
        }
        else {
