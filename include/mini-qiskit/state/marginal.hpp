...
) -> std::string
{
    ...
    const auto dyn_bitset = mqis::state_index_to_dynamic_bitset(i_state, n_qubits, input_endian);
...
inline auto rstrip_marginal_bits(const std::string& marginal_bitstring) -> std::string
{
    ...
    if (!impl_mqis::are_all_marginal_bits_on_side_<MBS::RIGHT>(marginal_bitstring)) {
...
{
    using MBS = impl_mqis::MarginalBitsSide;
    if (!impl_mqis::are_all_marginal_bits_on_side_<MBS::LEFT>(marginal_bitstring)) {
        auto err_msg = std::stringstream {};
    ...
    const auto it = std::find_if_not(
        ...
        [](auto bitchar) { return bitchar == impl_mqis::MARGINALIZED_QUBIT; }
    );
