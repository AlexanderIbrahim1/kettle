...
public:
    ...
    // NOLINTNEXTLINE(*explicit*)
    CircuitLogger(DensityMatrixCircuitLogger circuit_logger)
        : circuit_logger_ {std::move(circuit_logger)}
    {}
    ...
    [[nodiscard]]
    constexpr auto is_density_matrix_circuit_logger() const -> bool
    {
        return std::holds_alternative<DensityMatrixCircuitLogger>(circuit_logger_);
    }

    [[nodiscard]]
    constexpr auto get_density_matrix_circuit_logger() const -> const DensityMatrixCircuitLogger&
    {
        return std::get<DensityMatrixCircuitLogger>(circuit_logger_);
    }

private:
    std::variant<ClassicalRegisterCircuitLogger, StatevectorCircuitLogger, DensityMatrixCircuitLogger> circuit_logger_;
};
