...
TEST_CASE("compare density matrix u-gate simulations with statevector u-gate simulations")
{
    ...
    const auto testcase = GENERATE_COPY(
        TestCase {"H on 0, 1 qubit", "0", [](ket::QuantumCircuit& circ) { circ.add_u_gate(ket::h_gate(), 0); }},
        TestCase {"X on 0, 1 qubit", "0", [](ket::QuantumCircuit& circ) { circ.add_u_gate(ket::x_gate(), 0); }},
        TestCase {"H on 0, 2 qubits", "00", [](ket::QuantumCircuit& circ) { circ.add_u_gate(ket::h_gate(), 0); }},
        TestCase {"H on 1, 2 qubits", "00", [](ket::QuantumCircuit& circ) { circ.add_u_gate(ket::h_gate(), 1); }},
        TestCase {
        ...
        TestCase {"H on 0, 3 qubits", "000", [](ket::QuantumCircuit& circ) { circ.add_u_gate(ket::h_gate(), 0); }},
        TestCase {"H on 1, 3 qubits", "000", [](ket::QuantumCircuit& circ) { circ.add_u_gate(ket::h_gate(), 1); }},
        TestCase {"H on 2, 3 qubits", "000", [](ket::QuantumCircuit& circ) { circ.add_u_gate(ket::h_gate(), 2); }},
        TestCase {"H on each qubit, 3 qubits", "000", [](ket::QuantumCircuit& circ) { circ.add_u_gate(ket::h_gate(), {0, 1, 2}); }},
        TestCase {
            "bell(00+)",
            "00",
            [](ket::QuantumCircuit& circ)
...
TEST_CASE("compare density matrix primitive-gate simulations with statevector primitive-gate simulations")
{
    ...
    SECTION("2-qubit circuit, 1C1T gates")
    {
        ...
        const auto pair = GENERATE(std::pair<std::size_t, std::size_t> {0, 1}, std::pair<std::size_t, std::size_t> {1, 0});
