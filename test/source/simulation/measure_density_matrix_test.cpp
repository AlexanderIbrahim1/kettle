...
static void simulate_measurement_wrapper(StateKind& state, const ket::GateInfo& info, int measured_state)
{
...
TEST_CASE("simulate_measurement_()")
{
    ...
    SECTION("2 qubits; Hadamard on each")
    {
        auto testcase = GENERATE_REF(
            TestCase {.measured_qubit = 0, .measured_state = 0},
            TestCase {.measured_qubit = 1, .measured_state = 0},
            TestCase {.measured_qubit = 0, .measured_state = 1},
            TestCase {.measured_qubit = 1, .measured_state = 1}
        );
        ...
        auto density_matrix = ket::DensityMatrix {"00"};

        auto circuit = ket::QuantumCircuit {2};
        circuit.add_h_gate({0, 1});

        ket::simulate(circuit, statevector);
        ket::simulate(circuit, density_matrix);

        simulate_measurement_wrapper(statevector, info, testcase.measured_state);
        simulate_measurement_wrapper(density_matrix, info, testcase.measured_state);

        const auto expected = ket::statevector_to_density_matrix(statevector);
    ...
    SECTION("3 qubits")
    {
        auto testcase = GENERATE_REF(TestCase {0, 0}, TestCase {1, 0}, TestCase {2, 0}, TestCase {0, 1}, TestCase {1, 1}, TestCase {2, 1});

        // the measured bit doesn't matter for now
        const auto info = cre::create_m_gate(testcase.measured_qubit, 0);

        auto statevector = ket::Statevector {"000"};
        auto density_matrix = ket::DensityMatrix {"000"};
    ...
    SECTION("random")
    {
        ...
        SECTION("2 qubits")
        {
            ...
            auto statevector = ket::Statevector {testcase.initial_amplitudes};
            auto density_matrix = ket::statevector_to_density_matrix(statevector);

            simulate_measurement_wrapper(statevector, info, testcase.measured_state);
            simulate_measurement_wrapper(density_matrix, info, testcase.measured_state);

            auto expected_statevector = ket::Statevector {testcase.expected_amplitudes};
            auto expected_density_matrix = ket::statevector_to_density_matrix(expected_statevector);

            REQUIRE(ket::almost_eq(statevector, expected_statevector));
            REQUIRE(ki::almost_eq_with_print_(density_matrix, expected_density_matrix));
        }
        SECTION("3 qubits")
        {
