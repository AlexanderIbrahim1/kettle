...
{
    if (n_qubits == 0) {
        throw std::runtime_error {"UNREACHABLE: dev error, can't get number of single qubit gate pairs for 0 qubits\n"};
    }
...
auto number_of_double_qubit_gate_pairs_(std::size_t n_qubits) -> std::size_t
{
    ...
    if (n_qubits == 0) {
        throw std::runtime_error {"UNREACHABLE: dev error, can't get number of double qubit gate pairs for 0 qubits\n"};
    }
